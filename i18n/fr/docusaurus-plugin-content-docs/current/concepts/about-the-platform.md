...
>
>   > - The height of the block, the hash of the previous block, the timestamp> > of the block, the ID of the ecosystem where the block is located, and
>   >   > > the account address of the honor node of the block;
>   > - The position of the honor node in the platform parameter full_nodes> > array, the Merkel Root (MrklRoot) of all transactions in the block, and
...
> - The transaction time can neither be greater than the block generation time> nor be greater than the block generation time plus 600 seconds, and it can
...
indicated by the number <Highlight>1</Highlight> after the <Highlight>@</Highlight> symbol. When using application elements in the current
