...
127.0.0.1:7079 A full curl request with this additional data takes the form of
...
follows `"params":[0, "testname"]` - _Number_ null value is 0

If only the column value is passed, then the request parameters are passed as
follows `"params":[0,"", "title,page"]` - _String_ empty value for ""
...
receive the unique value and sign it. The temporary JWT token for getuid needs
to be passed in the request header. If the request is successful, the token
received in the response is contained in [**Authorization**](#authorization).
...
- **signature** - _String_ - Use the private key to sign the uid received by
  getuid.
...
User authentication status [Authorization](#authorization)
...
- List of transactions in the block and additional information for each
  ...
  - **key_id** - _Number_ - For the first block, it is the account address of
    the first block that signed the transaction.
...
role, no record will be returned. _title_ The data in the field will be replaced
...
- _Object_ - Get the actions request object
  ...
  - **lang** - _String_ - [Omitempty](#omitempty) -
    ...
    _en, tr_. If the specified multilingual resource is not found, e.g. _en-US_,
